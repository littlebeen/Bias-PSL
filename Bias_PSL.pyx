...
class Bias_PSLSVD(AlgoBase):
    ...
    def fit(self,trainset):
        ...
        pu = rng.normal(0, .01,
                        (trainset.n_users, K))
        qi = rng.normal(0, .01,  #以0为中心，上下浮动0.01
                        (trainset.n_items, K))
        ...
        for current_epoch in range(steps):
            ...
            for u, i, r in trainset.all_ratings():
                ...
                for f in range(K):
                    dot += qi[i, f] * pu[u, f]
                err = r - dot - bu[u] - bi[i] - global_mean
                X[n] += err
                T = KP * err + KI * X[n] + KD * (err - VV[n])
                VV[n] = err
                bu[u] += alpha * (T - beta * bu[u])
                bi[i] += alpha * (T - beta * bi[i])
                for f in range(K):
                    puf = pu[u, f]
                    qif = qi[i, f]
    ...
    def estimate(self, u, i):
        ...
        if known_user and known_item:
            est = np.dot(self.qi[i], self.pu[u]) + self.bu[u] + self.bi[i] + self.trainset.global_mean
        else:
